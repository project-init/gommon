...
func TestGetCorsHandlerAllowedOrigins(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "staging environment with multiple origins",
			env:            env.Staging,
			expectError:    false,
			allowedOrigins: []string{"https://app.staging.project-init.com", "https://api.staging.project-init.com"},
			urlMap:         urlMap,
		},
		{
		...
		{
			name:           "production environment with multiple origins",
			env:            env.Production,
			port:           8081,
			expectError:    false,
			allowedOrigins: []string{"https://app.production.project-init.com", "https://api.production.project-init.com"},
			urlMap:         urlMap,
		},
		{
